...
class HttpClient(object):
    ...
    def _validate_oauth(self):
        ...
        created_at = config['oauth.created_at'] 
        expires_in = config['oauth.expires_in']
        #expires_minus = (48 * 60 * 60)
        #expires_at = config['oauth.created_at'] + config['oauth.expires_in'] - (48 * 60 * 60)
        #expires_at = created_at + expires_in - expires_minus

        #2025-05-30 - psyciknz Changed token expiry calculation 
        expired = (time.time()-created_at) >=  expires_in
